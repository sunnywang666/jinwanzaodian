...
- **修早上上菜时豆浆偏大**：早点图里豆浆是竖图（宽高比 0.56，杯子），其它都是横图（约 1.2）。场景按固定宽度 12% 渲染、高度自适应，竖图就被拉得特别高。给场景菜品图加 `aspectRatio: 1.15 + object-contain`，竖的豆浆按高度收进框，和其它早点大小一致。
- **打包文件名去掉 debug 字样**：产物改为 `今晚早点.apk` / `今晚早点-演示.apk`（仍是 assembleDebug 的 debug 签名包、可直接安装，只是文件名更干净；要 release 包需配签名密钥，对作业无必要）。
- 更新 `package.json` 版本号为 `6.47.0`。

## v6.46

演示面板 + 聊天记录持久化 + 一批体验修复。

- **演示面板替代调试面板**（仅演示版）：首页右上「演示」开一个干净的演示导航，直接跳 清晨开门 / 午间过场 / 傍晚预承诺 / 夜晚打烊 + 重看新手导览。移除原时间模拟滑块、热点虚线框（要 debug 时再单独挂回 `TimeSimPanel`）；首页时钟的调试拖动也随之关闭=位置固定。
- **顶部按钮收成一行**：右上只有「演示 + 设置」；**重置移进设置页**（设置里本就有"清空所有数据"），不再有飘在外面的重置、不再三行。
- **事件后能看到对应时段背景**：演示跳转会暂停自动场景 30 分钟（拿掉时间模拟后 `getNow` 用真实时间，否则一回首页就被切回），所以走完傍晚/夜晚等事件回到首页能看到对应背景。
- **图鉴背景跟时段**：菜谱本/客人本的确认页与翻开页背景改用当前时段（`sceneByDemo[场景]`），不再写死白天图。
- **滚动条美化**：细、暗、半透明、无上下箭头（全局 `::-webkit-scrollbar`），替掉默认那条白底带箭头。
- **傍晚预承诺 ↔ 精灵对话联动**：从傍晚写完心事进对话，精灵**顺着刚写的心事回应 + 给对症方法**（断网兜底用按人格的 `evening.method` 文案）；不再让用户重写心事（开场第二句改、快捷键"写心事"→"再改改心事"）；傍晚保存后底部给两个明确出口——`回铺子` / `和{精灵}聊聊`。
- **聊天记录持久化**：新增 `chatStore.ts`，对话存 localStorage（演示/正式分键、留最近 50 条），关掉再开接着上次聊；`resetAll` 一并清除。也是后续"精灵从聊天里总结心事"的数据基础。
- 验证：tsc + 45 单测全过。更新 `package.json` 版本号为 `6.46.0`。

## v6.45

修新手导览在宽屏溢出：把整个导览套进和首页一模一样的固定手机列（`max-w-[430px] + overflow-hidden`）。

- 之前导览用 `fixed inset-0` 全屏、没套首页那个固定列，导致超出 1024:1536 舞台的物件（留言板 x63+宽46、小屋 x67+宽38）溢出到屏幕右侧、且没被蒙版盖住（宽屏/浏览器下尤其明显）。现与首页同一容器，超出部分被裁掉、坐标一致；精灵/气泡/跳过键也收进该列、宽屏居中。
- 时钟默认坐标确认固定为 L45.1/T13.8/W12.7（v6.42 已写死），新装即到位。
- 验证：tsc + 45 单测全过。更新 `package.json` 版本号为 `6.45.0`。
